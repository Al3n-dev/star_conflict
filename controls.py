...
def inos_check(stats, screen, sc, gun, inos, bullets):
    """Проверка, добрались ли вагины до края экрана"""
...
def create_army(screen, inos):
    """"Создание армии вагин"""
    ino = Ino(screen)
    ino_width = ino.rect.width
    number_ino_x = int((1200 - 2 * ino_width) / ino_width)
    ino_heidht = ino.rect.height
    number_ino_y = int((800 - 100 - 2 * ino_heidht) / ino_heidht)

    for row_number in range(number_ino_y - 1):
        for ino_number in range(number_ino_x):
            ino = Ino(screen)
            ino.x = ino_width + (ino_width * ino_number)
            ino.y = ino_heidht + (ino_heidht * row_number)
            ino.rect.x = ino.x
